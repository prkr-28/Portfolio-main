  ...
  switch (action.type) {
    ...
    case "NEXT_WORD":
      return {
        ...state,
        deleting: false,
        idx: (state.idx + 1) % action.payload,
      };
    default:
      return state;
  }
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {error && type === "email" && (
        <span className="text-xs text-red-500">
          Invalid email. Needs 6+ chars before @, correct formatting, and a
          popular provider.
        </span>
      )}
    </div>
...
const Toast = ({ show, onClose }) => {
  ...
  return (
    <div
      id="toast"
      className="fixed top-11 left-6 z-50 flex items-center gap-3 bg-[#040404] border border-green-400/40 rounded px-5 py-3"
    >
      ...
      <button
        type="button"
        onClick={onClose}
        className="ml-2 text-gray-500 hover:text-gray-50 transition-colors text-xs"
      >
        ✕
      </button>
    </div>
...
const Contact = () => {
  ...
  const validateEmail = (email) => {
    const regex =
      /^(?=.{6,}@)[a-zA-Z0-9]+([._-][a-zA-Z0-9]+)*@(gmail\.com|proton\.me|protonmail\.com|outlook\.com|yahoo\.com|icloud\.com)$/i;
      ...
      <div className="relative z-10 w-full max-w-6xl flex flex-col gap-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div ref={leftRef} className="flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {contactInfo.map(({ icon, label, value, href }) => (
                <a
                  ...
                  rel="noreferrer"
                  className={`group flex items-center gap-3 p-4 rounded border border-gray-50/5 bg-gray-50/2 transition-all duration-300 hover:border-green-400/40 hover:bg-green-400/5 ${href ? "cursor-pointer" : "cursor-default"}`}
            ...
            {/* NOTE */}
            <div
              className="flex items-center gap-3 p-4 rounded border border-green-400/20 bg-green-400/5"
              style={{ backdropFilter: "blur(8px)" }}
            >
            ...
            <div className="h-82 rounded overflow-hidden relative">
              <Map center={[85.7437, 20.2949]} zoom={13} theme="dark">
                <MapControls
                  position="top-right"
                  showZoom
                  showCompass
            ...
            >
              ...
              <form onSubmit={sendEmail} className="p-6 flex flex-col gap-5">
                ...
                <button
                  type="submit"
                  disabled={sending}
                  className="group relative w-full py-3.5 mt-1 text-sm font-bold uppercase tracking-widest text-black bg-green-500 rounded overflow-hidden transition-all disabled:opacity-60 disabled:cursor-not-allowed aldrich-regular"
                >
