...
const GithubContribution = async () => {
  ...
  try {
    ...
    if (contribRes.ok) {
      ...
      totalContributions =
        (data.total as Record<string, number>)?.["2026"] ?? 0;
    }
  } catch {
    /* fail gracefully */
  }
  ...
  const statCards = [
    ...
    {
      label: "Public Repos",
      value: fmt(user?.public_repos ?? repos.length),
      accent: "#a8e6cf",
    },
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                <div className="relative shrink-0">
                  ...
                  <img
                    src={
                      user?.avatar_url ?? `https://github.com/${USERNAME}.png`
                    }
                    alt={`${USERNAME} avatar`}
                    width={88}
                    height={88}
                ...
                <div>
                  ...
                  {user?.bio && (
                    <p className="mt-1.5 max-w-sm text-sm font-medium text-black/70 dark:text-white/80 leading-snug">
                      {user.bio}
                    </p>
                  )}
                </div>
              ...
              {statCards.map((item) => (
                <div key={item.label} className="relative">
                  ...
                  >
                    ...
                    <p className="mt-1 text-lg font-black text-black leading-none sm:text-xl">
                      {item.value}
                    </p>
                  </div>
                </div>
              ...
              <div className="relative rounded-xl border-[3px] border-black dark:border-[#a8e6cf] bg-[#fafafa] dark:bg-stone-900 p-5 sm:p-7">
                ...
                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-black/40 dark:text-white/50">
                      Less
                    </span>
                    {[0, 1, 2, 3, 4].map((lvl) => (
                      <div
                ...
                {weeks.length > 0 ? (
                  ...
                  <div className="flex h-36 items-center justify-center rounded-lg border-2 border-dashed border-black/20 dark:border-white/30 bg-white dark:bg-stone-800">
                    <p className="text-sm font-bold text-black/40 dark:text-white/60">
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {[
                    ...
                    {
                      label: "Peak Day",
                      value: `${Math.max(...contributions.map((d) => d.count), 0)}`,
                      unit: "commits",
                      bg: "#66dff4",
                    },
                  ].map((stat) => (
                    <div key={stat.label} className="relative">
                      ...
                      >
                        <p className="text-[9px] font-black uppercase tracking-widest text-black/50 leading-none">
                          {stat.label}
                        </p>
                        <p className="mt-1.5 text-2xl font-black text-black leading-none">
                          {stat.value}
                        </p>
                        <p className="mt-0.5 text-[10px] font-bold text-black/50">
                          {stat.unit}
                        </p>
                      </div>
                    </div>
